// ***For all the exercises, be sure to log the output of the function to the console.***
    ...
    */
    ...
    // I still do not have this functioning where I can pass out the invalid chess pieces in the given array...
    // console.log(chessCalc([king, queen, pawn, pawn, pawn, bishop, android]));
    // There is a way I can use a switch statement to discard invalid chess pieces...
    // switch(p) {
    //     case "pawn":
    //         break;
    //     case "rook":
    ...
    */
    ...
    const number = 4;
    mapObj.set(number, "number");
    const nullVal = null;
    mapObj.set(nullVal, "null");

    console.log(mapObj.has(myObject));
    console.log(mapObj.has(string));
    console.log(mapObj.has(boolean));
    console.log(mapObj.has(number));
    console.log(mapObj.has(nullVal));
    console.log(mapObj);

    //28.(a) The above console.log() statment returns false.  Write another console.log() statement explaining why this line of code prints false.  Refactor the code `mapObj.set()`, so the code : `mapObj.has() returns true.  The goal is to successfully check and see if {company : "TEKsystems"} exists in the mapObj.

    //your code...
    console.log(`The mapObj.set() does not store a reference to the value, and if there is no reference then it returns false. We can remedy this by setting the values before we set them. Map.prototype.has(key) returns a boolean asserting whether a value has been associated to the key in the Map object or not.`)
    ...
    //your code...
    let adding = (x,y) => ( "adding : " + (x + y) );
    let subtracting = (x,y) => ( "subtracting : " + (x - y) );
    let multiplying = (x,y) => ( "multiplying : " + (x * y) );
    let dividing = (x,y) => ( "dividing : " + (x / y) );
    
    // The mathematical function expressions console.log the desired output
    // add(4,4);
    // subtract(4,4);
    // multiply(4,4);
    // divide(4,4);

    // How do I randomly pick a mathematical function expression and then return the computed value of the operation performed
    let operations = [adding, subtracting, multiplying, dividing];
    function doMath(arr) {
        for(let i = 0; i < arr.length; i ++) {
            console.log(arr[i](4,4));
        }
    };
    doMath(operations);
    ...
    function stockGain(basis) {
        let message = " is how much the stock market has increased";
        function go(yrs) {
            let r = .05;
            console.log(r + message);
        };
    };
    console.log(stockGain(5));
    ...
    //your code...
    let futureValue = stockGain(1500, 5);
